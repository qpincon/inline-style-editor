...
| cssRuleFilter      | Function      |   A function taking a DOM element and the CSS selector text for the rule, that should return `false` if the rule should not display in the editor. The inline style rule is also passed through this filter with the selector text `"inline"`. If all rules (including inline) are filtered out, the editor will not open.  |
